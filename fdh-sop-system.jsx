...
export default function SOPSystem() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("fdh-sops-v2");
        setSops(r ? JSON.parse(r.value) : SEED);
      } catch { setSops(SEED); }
      setLoaded(true);
    })();
  }, []);
  ...
  useEffect(() => {
    if (!loaded) return;
    window.storage.set("fdh-sops-v2", JSON.stringify(sops)).catch(() => {});
  }, [sops, loaded]);
  ...
  async function doSearch() {
    ...
    setSearching(true);
    try {
      const idx = sops.map(s => ({ id: s.id, title: s.title, category: s.category, tags: s.tags.join(", "), snippet: s.content.slice(0, 180) }));
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 300,
          messages: [{ role: "user", content: `You are searching a dental office SOP library. Query: "${searchQ}". Rank these SOPs by relevance (most relevant first). Return ONLY a JSON array of IDs. SOPs: ${JSON.stringify(idx)}` }],
        }),
      });
